...
const ChciNabidku = () => {
  ...
  const btnSubmit = async (e) => {
    e.preventDefault();
    const regex1a =
      /^[A-ZÄÖÜßÁČĎÉĚÍŇÓŘŠŤÚŮÝŽ][a-zäöüßáčďéěíňóřšťúůýž]{2,} [A-ZÄÖÜßÁČĎÉĚÍŇÓŘŠŤÚŮÝŽ][a-zäöüßáčďéěíňóřšťúůýž]{1,}$/;
    const regex1b =
      /^[A-ZÄÖÜßÁČĎÉĚÍŇÓŘŠŤÚŮÝŽ][a-zäöüßáčďéěíňóřšťúůýž]{2,} [A-ZÄÖÜßÁČĎÉĚÍŇÓŘŠŤÚŮÝŽ][a-zäöüßáčďéěíňóřšťúůýž]{1,} [A-ZÄÖÜßÁČĎÉĚÍŇÓŘŠŤÚŮÝŽ][a-zäöüßáčďéěíňóřšťúůýž]{1,}$/;
    const regex2a = /^[0-9]{9}$/;
    ...
    if (formData2.name === "") {
      ...
      formData2.name !== "" &&
      !(regex1a.test(formData2.name) || regex1b.test(formData2.name))
    ) {
      ...
      formData2.phone !== "" &&
      !(regex2a.test(formData2.phone) || regex2b.test(formData2.phone))
    ) {
    ...
    // Dobře zadaný telefon a email
      ...
      try {
        ...
        setErrorFaze(
          "Nabídka nebyla přidána, kontaktujte naší firmu " + err.message
        );
      }
    }
  };
  ...
  // -----------------------------------------------------
  return (
    <section className="chci-nabidku">
      {/* Zobrazený formulář */}
      {zobrazeniFormulare && (
        <article className="odesilaci-formular">
          <h1>
            Zadejte{" "}
            {prvniFazeForm ? " informace o Vaší nemovitosti" : "své údaje"}
          </h1>
          <br />
          <form>
            {/* První fáze */}
          ...
          <div className="pod-Form">
            <p>{errorFaze}</p>
            <button className="btn-dalsiFaze" onClick={kontrolaFaze1}>
              {prvniFazeForm ? "další" : "Zpět"}
            </button>
          </div>
        </article>
      )}
